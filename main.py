...
def variant1(token):
    response = get('https://discord.com/api/v10/auth/login', headers={"Authorization": token})#Bad variant for mass token check due to the rate limit.
    return True if response.status_code == 200 else False

def variant2(token):
    response = post(f'https://discord.com/api/v10/invite/{randint(1,9999999)}', headers={'Authorization': token})
